            ...
            <DialogTitle className="text-2xl font-bold text-foreground flex items-center gap-3">
              <Avatar className="w-12 h-12 ring-2 ring-[#1B4FFF]/20">
                <AvatarFallback className="bg-gradient-to-br from-[#1B4FFF] to-[#0A1E39] text-white font-semibold text-lg">
                  {client.contactPerson?.avatar || 'CL'}
                </AvatarFallback>
              </Avatar>
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    <Building2 className="w-4 h-4 text-[#1B4FFF]" />
                    <div>
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="text-center p-4 bg-white rounded-lg border border-blue-200">
                  <div className="text-2xl font-bold text-blue-700">{client.projects?.total || 0}</div>
                  <div className="text-sm text-blue-600">{dir === 'rtl' ? 'إجمالي المشاريع' : 'Total Projects'}</div>
                </div>
                <div className="text-center p-4 bg-white rounded-lg border border-green-200">
                  <div className="text-2xl font-bold text-green-700">{client.projects?.active || 0}</div>
                  <div className="text-sm text-green-600">{dir === 'rtl' ? 'مشاريع نشطة' : 'Active Projects'}</div>
                </div>
                <div className="text-center p-4 bg-white rounded-lg border border-purple-200">
                  <div className="text-2xl font-bold text-purple-700">{client.projects?.completed || 0}</div>
                  <div className="text-sm text-purple-600">{dir === 'rtl' ? 'مشاريع مكتملة' : 'Completed Projects'}</div>
                </div>
                <div className="text-center p-4 bg-white rounded-lg border border-orange-200">
                  <div className="text-2xl font-bold text-orange-700">{client.projects?.onHold || 0}</div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-4 bg-white rounded-lg border border-green-200">
                  ...
                  <div className="text-2xl font-bold text-green-700">{formatCurrency(client.totalValue || 0)}</div>
                </div>
                <div className="p-4 bg-white rounded-lg border border-blue-200">
                  ...
                  <div className="text-2xl font-bold text-blue-700">{formatCurrency(client.paidValue || 0)}</div>
                </div>
                <div className="p-4 bg-white rounded-lg border border-orange-200">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm text-muted-foreground">{dir === 'rtl' ? 'المبلغ المستحق' : 'Outstanding'}</span>
                    <Clock className="w-4 h-4 text-orange-600" />
                  </div>
                  <div className="text-2xl font-bold text-orange-700">
                    {formatCurrency((client.totalValue || 0) - (client.paidValue || 0))}
                  </div>
                </div>
              </div>
            </CardContent>
            ...
            <Card className="bg-gradient-to-r from-gray-50 to-slate-50 border-gray-200">
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {(dir === 'rtl' ? client.tags : client.tagsEn || client.tags).map((tag: string, index: number) => (
                    <Badge
                      key={index}
                      variant="secondary"
