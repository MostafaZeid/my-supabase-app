...
serve(async (req) => {
  ...
  try {
    ...
    switch (method) {
      case 'GET':
        if (ticketId && ticketId !== 'tickets_api_2025_12_17_00_00') {
          // Get single ticket
          const { data, error } = await supabaseClient
            ...
            .select(`
              *,
              ...
              project:projects_2025_12_17_00_00(id, name, name_en)
            `)
            .eq('id', ticketId)
            .single();
      ...
      case 'POST':
        ...
        return new Response(
          JSON.stringify({ success: true, data: newTicket }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      ...
      case 'PUT':
        ...
        return new Response(
          JSON.stringify({ success: true, data: updatedTicket }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
